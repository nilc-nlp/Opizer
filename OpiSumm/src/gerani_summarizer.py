...
class Gerani_Summarizer(object):
    ...
    def __select_template_quantifier(self, templates, aspect, absolute_quantifier, relative_quantifier, lowercase):
        ...
        #for match in re.findall("(<([^>,]+),([^>,]+)>)", template_quantifier):
        #    if absolute_quantifier == 1:  template_quantifier = re.sub(match[0], match[1], template_quantifier)
        #    else: re.sub(match[0], match[2], template_quantifier)
        #"0.0": ["<NUMBER> <usuário,usuários> <comentou,comentaram> sobre <o,os,a,as> <ASPECT> e em geral eles ", "<NUMBER> <comprador,compradores> <mencionou,mencionaram> <o,os,a,as> <ASPECT> e eles "]
