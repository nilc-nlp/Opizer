...
class Opizere_Summarizer(object):
    ...
    def create_summary(self, folder_path, number_aspects, number_sentences, write=True):
        ...
        for (aspect, data) in top_aspects[:number_aspects]:
            ...
            text += "          <i>Sentenças Positivas: <font color='green'>%s</i></font>\n" % pos_size
            ...
            text += "          <i>Sentenças Negativas: <font color='red'>%s</font></i>\n" % neg_size
    ...
    def __get_sentence_ranking(self, aspect, polarity, alpha=0.23):
        ...
        for id_sentence in sentence_by_proximity.keys():
            raking_list[id_sentence] = alpha * sentence_by_proximity[id_sentence] + (1.0 - alpha) * sentence_by_position[id_sentence]
